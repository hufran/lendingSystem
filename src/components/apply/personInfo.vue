...
export default{
  data(){
    return {
      data:[
        {name:"姓名",alias:"name",value:"",placeHolder:"请输入姓名",type:"text",input:true,require:true,regex:/^[\u4e00-\u9fa5]+((·|•|●)[\u4e00-\u9fa5]+)*$/i,length:10,empty:"用户姓名不能为空!",err:"姓名格式不正确!"},
        ...
        {name:"年收入",alias:"yearIncome",value:"",placeHolder:"请输入年收入",type:"number",input:true,require:true,regex:/^((0\.\d?)||([1-9]\d*(\.\d*[1-9])?))+$/i,empty:"年收入不能为空!",err:"年收入应该大于0!"},
